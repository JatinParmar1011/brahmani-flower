...
import { getProfile, updateProfile, getAddresses, createAddress, updateAddress, setDefaultAddr, deleteAddress } from '../services/authService';
    ...
    <div className={`border rounded-2xl overflow-hidden transition-all duration-300 bg-white ${
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-3">
          <div>
            <p className="font-bold text-gray-900 text-sm">{o.orderNumber || `#${o.id}`}</p>
            <p className="text-xs text-gray-400 mt-0.5">
              {o.createdAt ? new Date(o.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : ''}
            </p>
          </div>
          <span className={`text-xs font-semibold px-3 py-1 rounded-full ${statusStyle[o.status] || 'bg-gray-100 text-gray-600'}`}>
            {o.status?.replace('_', ' ')}
          </span>
        </div>
        <p className="text-sm text-gray-700 mb-3">
          {o.items?.length > 0 ? `${o.items[0].productName}${o.items.length > 1 ? ` +${o.items.length - 1} more` : ''}` : '—'}
        </p>
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <span className="text-sm font-bold text-gray-900">₹{o.totalAmount}</span>
          <button onClick={() => setOpen(v => !v)}
            className="flex items-center gap-1 text-xs text-[#1a6b8a] font-semibold hover:underline">
      ...
      {open && (
        <div className="border-t border-[#1a6b8a]/20 bg-gradient-to-br from-[#1a6b8a]/4 to-blue-50/60 px-5 py-5">
          <div className="grid grid-cols-2 gap-4 mb-4">
            {o.items?.map((item, i) => (
              <div key={i} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wide mb-1">Item {i + 1}</p>
                <p className="text-sm font-semibold text-gray-800">🌸 {item.productName}</p>
                <p className="text-xs text-gray-500 mt-0.5">Qty: {item.quantity} × ₹{item.unitPrice}</p>
                <p className="text-xs font-bold text-gray-700 mt-0.5">₹{item.subtotal}</p>
              </div>
            ))}
          ...
          <div className="flex items-center gap-2 bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm">
            ...
            <div>
              <p className="text-xs font-bold text-gray-700">Status: {o.status?.replace('_', ' ')}</p>
            </div>
          </div>
        </div>
      )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-3 mt-6">
              <button onClick={() => load(page - 1)} disabled={page === 0}
                className="px-4 py-2 text-xs rounded-lg border border-gray-200 text-gray-500 hover:border-[#1a6b8a] hover:text-[#1a6b8a] disabled:opacity-40 transition-colors">
                ← Prev
              </button>
              <span className="text-xs text-gray-400 self-center">Page {page + 1} of {totalPages}</span>
...
// ── Address Book Panel ─────────────────────────────────────────────────────
const TAG_ICONS = { HOME: '🏠', WORK: '🏢', OTHER: '📌' };
...
function AddressBookPanel() {
  ...
  const formInitial = editAddress ? {
    fullName: editAddress.fullName, mobileNumber: editAddress.mobileNumber,
    address1: editAddress.address1, address2: editAddress.address2 || '',
    city: editAddress.city, state: editAddress.state, pincode: editAddress.pincode,
    country: editAddress.country, addressType: editAddress.addressType,
        ...
        <div className="grid grid-cols-2 gap-4">
          {addresses.map(a => (
            <div key={a.id} className={`relative border-2 rounded-2xl p-5 transition-all ${
              a.isDefault ? 'border-[#1a6b8a] bg-[#1a6b8a]/3 shadow-sm' : 'border-gray-100 bg-white hover:border-[#1a6b8a]/30 hover:shadow-sm'
            }`}>
              {a.isDefault && (
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="text-lg">{TAG_ICONS[a.addressType] || '📌'}</span>
                <span className="text-sm font-bold text-gray-800">
                  {a.addressType.charAt(0) + a.addressType.slice(1).toLowerCase()}
                </span>
              </div>
              <p className="text-sm font-semibold text-gray-800">{a.fullName}</p>
              <p className="text-xs text-gray-500 mt-0.5">📞 +91 {a.mobileNumber}</p>
          ...
          {addresses.length === 0 && !adding && (
            <div className="col-span-2 text-center py-12 text-gray-400">
              ...
              <p className="text-xs mt-1">Add your first delivery address</p>
            </div>
          )}
        </div>
      )}
...
const STATUS_STEP = {
  PENDING_PAYMENT: 0, CONFIRMED: 1, PROCESSING: 2, SHIPPED: 3, DELIVERED: 4, CANCELLED: -1,
};
...
function TrackOrderPanel() {
  ...
  const handleTrack = async () => {
    ...
    setLoading(true); setError(''); setTracked(null);
    try {
      const data = await fetchMyOrders(0, 50);
      const found = (data.content ?? []).find(
        o => o.orderNumber === orderNum.trim() || String(o.id) === orderNum.trim()
      );
      if (!found) { setError('Order not found. Please check the order number.'); return; }
      setTracked(found);
    } catch {
  ...
  const step = tracked ? (STATUS_STEP[tracked.status] ?? 0) : 0;
  const cancelled = tracked?.status === 'CANCELLED';

  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-1">Track Order</h2>
      <p className="text-sm text-gray-400 mb-7">Enter your order number to track delivery status</p>

      <div className="flex gap-3 mb-8">
        <input type="text" value={orderNum} onChange={e => setOrderNum(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && handleTrack()}
          placeholder="Enter Order Number (e.g. BF250101xxxxx)"
        ...
        <button onClick={handleTrack} disabled={loading}
          className="bg-[#1a6b8a] hover:bg-[#155a75] disabled:bg-gray-300 text-white font-semibold px-6 py-3 rounded-xl text-sm transition-colors">
          {loading ? '…' : 'Track'}
        </button>
      </div>
        ...
        <div className="bg-gradient-to-br from-[#1a6b8a]/5 to-blue-50 rounded-2xl p-6 border border-[#1a6b8a]/10">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="font-bold text-gray-900">{tracked.orderNumber}</p>
            </div>
            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${
              cancelled ? 'bg-red-100 text-red-600' : 'bg-[#1a6b8a]/10 text-[#1a6b8a]'
            }`}>{tracked.status?.replace('_', ' ')}</span>
          </div>
          ...
          {cancelled ? (
            ...
            <div className="relative">
              ...
              <div className="absolute top-4 left-4 h-0.5 bg-[#1a6b8a] z-0 transition-all duration-700"
                style={{ width: `${(step / (TRACK_STEPS.length - 1)) * (100 - 8)}%` }} />
              <div className="relative z-10 flex justify-between">
                {TRACK_STEPS.map((s, i) => (
                  <div key={s} className="flex flex-col items-center gap-2" style={{ width: '20%' }}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${
                      i < step ? 'bg-[#1a6b8a] border-[#1a6b8a] text-white' :
                      i === step ? 'bg-white border-[#1a6b8a] text-[#1a6b8a] shadow-md ring-4 ring-[#1a6b8a]/20' :
                      'bg-white border-gray-300 text-gray-400'
                    }`}>{i < step ? '✓' : i + 1}</div>
                    <p className={`text-[10px] text-center leading-tight font-medium ${i <= step ? 'text-[#1a6b8a]' : 'text-gray-400'}`}>{s}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {!tracked && !error && (
        <div className="text-center py-12 text-gray-300">
          ...
          <p className="text-sm font-medium text-gray-400">Enter an order number above to track your delivery</p>
        </div>
      )}
    </div>
  );
