...
// ── Main CartPage ─────────────────────────────────────────────────────────
export default function CartPage({ onBack, onViewOrders, onSignIn, onCheckout }) {
  const { items, cartTotal, loading, toast, updateQty, removeItem, reload } = useCart();
  const { user } = useAuth();
  ...
  const toggleAddon = (addon) =>
    setAddedOns(prev => prev.includes(addon.id) ? prev.filter(id => id !== addon.id) : [...prev, addon.id]);
      ...
      <div className="max-w-[1300px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{productCount} product{productCount !== 1 ? 's' : ''} in your cart</p>
          </div>
        </div>

        {toast && (
          <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 text-white text-sm px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 ${
        ...
        {removed && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-gray-900 text-white text-sm px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3">
            ...
            <button onClick={() => setRemoved(null)} className="text-[#4dd0e1] font-bold hover:underline">Dismiss</button>
          </div>
        )}
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {items.map((item, idx) => {
                  ...
                  return (
                    <div key={item.id}
                      className={`flex items-center gap-5 px-6 py-5 ${idx !== items.length - 1 ? 'border-b border-gray-100' : ''}`}>
                      <div className="w-20 h-20 rounded-xl bg-gradient-to-br from-[#1a6b8a]/10 to-[#1a6b8a]/5 flex items-center justify-center flex-shrink-0 overflow-hidden">
                        {item.imageUrl
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          {item.tag && <span className="text-[10px] font-bold bg-[#1a6b8a]/10 text-[#1a6b8a] px-2 py-0.5 rounded-full">{item.tag}</span>}
                        </div>
                        <h3 className="text-sm font-bold text-gray-900">{item.name}</h3>
                        {item.stockWarning && (
                          <p className="text-[11px] text-amber-600 font-semibold mt-0.5">⚠️ Only {item.availableStock} left in stock</p>
                        )}
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button onClick={() => handleUpdateQty(item.id, -1)}
                          className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-[#1a6b8a] hover:text-white hover:border-[#1a6b8a] transition-all font-bold text-lg leading-none">−</button>
                      ...
                      <button onClick={() => handleRemove(item.id)}
                        className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-300 hover:bg-red-50 hover:text-red-500 transition-all flex-shrink-0">✕</button>
                    </div>
                  );
                })}
              </div>
                  ...
                  {ADDONS.map(addon => {
                    ...
                    return (
                      <div key={addon.id}
                        ...
                        <div className="h-24 bg-gradient-to-br from-orange-50 to-amber-50 flex items-center justify-center text-5xl">{addon.emoji}</div>
                        <div className="p-3">
                          ...
                          <button onClick={() => toggleAddon(addon)}
                            className={`w-full text-xs font-semibold py-1.5 rounded-lg border transition-all ${
                              added ? 'bg-[#1a6b8a] text-white border-[#1a6b8a]' : 'bg-white text-[#1a6b8a] border-[#1a6b8a] hover:bg-[#1a6b8a] hover:text-white'
                            }`}>
              ...
              <div className="relative">
                ...
                <div className="bg-white rounded-2xl border-2 border-green-400 shadow-sm pt-7 pb-6 px-6">
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <span className="text-xs text-gray-400 font-medium">{productCount} Product{productCount !== 1 ? 's' : ''}</span>
                  </div>
                  <div className="flex flex-col gap-3 text-sm">
                    <div className="flex justify-between items-center text-gray-600">
                      <span>Order Total</span>
                      <div className="flex items-center gap-2">
                        {saved > 0 && <span className="text-gray-400 line-through text-xs">₹{originalTotal}</span>}
                  ...
                  <button
                    ...
                    className="mt-5 w-full bg-gradient-to-r from-[#1a6b8a] to-[#155a75] hover:from-[#155a75] hover:to-[#0f4a62] text-white font-bold py-3.5 rounded-xl transition-all shadow-md hover:shadow-lg active:scale-[0.98] text-sm tracking-wide">
